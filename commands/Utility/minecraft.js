...
module.exports = {
 ...
 run: async (client, message, args) => {
  try {
   ...
   if(!(/^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,}(:?[0-9]*)$/igm.test(args[0]))) {
   ...
   if(args[1] == "--bedrock" || args[1] == "bedrock") {
    ...
    .then((body) => {
     ...
     const embed = new MessageEmbed() // Prettier
      .setAuthor(`${args[0]} (Bedrock)`, `https://api.mcsrvstat.us/icon/${args[0]}`)
      ...
      .setFooter(
       ...
       if(body.motd && body.motd.raw[0]) embed.setDescription(">>> " + body.motd.raw[0].replace(/§[0-9A-FK-OR]/ig,'').replaceAll("`", ""))
   ...
   fetch(`https://api.minetools.eu/ping/${args[0].replace(":", '/')}`)
    ...
    .then((body) => {
     ...
     const embed = new MessageEmbed() // Prettier
      .setAuthor(args[0], `https://api.mcsrvstat.us/icon/${args[0].replace(":", '/')}`)
