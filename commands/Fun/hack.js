   ...
   if (chance >= 0.4) {
    return message.channel
     .send({
      embed: {
       color: 16734039,
       description: "Hacking" + member.user.username,
      },
     })
     .then((msg) => {
      setTimeout(function () {
       msg.edit({ embed: { color: 16734039, title: member.user.username, description: hacks[Math.floor(Math.random() * hacks.length)] } });
      }, randomDelay);
     })
     .catch(console.error);
   } else {
    return message.channel
     ...
     .then((msg) => {
      setTimeout(function () {
       msg.edit({ embed: { color: 16734039, title: member.user.username, description: fails[Math.floor(Math.random() * fails.length)] } });
      }, randomDelay);
     });
   }
  } catch (err) {
