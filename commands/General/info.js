 ...
 run: async (client, message, args) => {
  try {
   ...
   const embed = new Discord.MessageEmbed() // Prettier()
    ...
    .addField("<:ram:856174395508391986> RAM Usage (BOT)", "```" + (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2) + "MB/" + osutils.totalmem().toString().split(".")[0] + "." + osutils.totalmem().toString().split(".")[1].split("")[0] + osutils.totalmem().toString().split(".")[1].split("")[1] + "MB " + `(${Math.round(((process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2) / osutils.totalmem().toString().split(".")[0] + "." + osutils.totalmem().toString().split(".")[1].split("")[0]) * 100)}%)` + "```")
