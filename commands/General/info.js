  ...
  try {
   ...
   cpu.usage().then((cpupercentage) => {
    const embed = new Discord.MessageEmbed() // Prettier()
     ...
     .addField("🏓 Ping", `\`\`\`Bot: ${Math.round(client.ws.ping)} | API: ${(Date.now() - message.createdTimestamp).toString().replace(/-/g, "")}`)
     .addField("<:cpu:856174395436171294> CPU", "```" + cpu.model() + " (" + cpu.count() + " cores)" + " [" + cpupercentage + "% used / " + proc.totalProcesses() + " proc. running]```")
