 ...
 client.on("ready", async () => {
  ...
  if (web_config.web.ssl_certs == true) {
   ...
   https.createServer(http_options, app).listen(port, null, null, () => {
    if (process.argv.includes(`--dashboard`)) console.log(chalk.bold(chalk.bold.magenta("> ") + chalk.blue.bold("[DASH]")) + chalk.cyan.bold(` Dashboard is up and running on url ${domain}${port}!`));
    if (process.argv.includes(`--api`)) console.log(chalk.bold(chalk.bold.red("> ") + chalk.blue.bold("[API]")) + chalk.cyan.bold(` API is up and running on url ${domain} and port ${port}!`));
   });
  } else {
   app.listen(port, null, null, () => {
    if (process.argv.includes(`--dashboard`)) console.log(chalk.bold(chalk.bold.magenta("> ") + chalk.blue.bold("[DASH]")) + chalk.cyan.bold(` Dashboard is up and running on url ${domain}${port == 8080 ? "" : `:${port}`} !`));
    if (process.argv.includes(`--api`)) console.log(chalk.bold(chalk.bold.red("> ") + chalk.blue.bold("[API]")) + chalk.cyan.bold(` API is up and running on url ${domain}${port == 8080 ? "" : `:${port}`} !`));
   });
  }
 });
};
