import { Menu as DefaultMenu, MenuButton as DefaultMenuButton, MenuItem as DefaultMenuItem, MenuItems as DefaultMenuItems, MenuButtonProps, MenuItemProps, MenuItemsProps, MenuProps, Transition } from "@headlessui/react";
...
export const MenuItems = React.forwardRef<React.ElementRef<typeof DefaultMenuItems>, MenuItemsProps>(({ className, ...props }, ref) => (
 <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
  <DefaultMenuItems ref={ref} className={cn("absolute right-0 z-50 mt-2 w-56 origin-top-right divide-y divide-neutral-800 rounded-xl border border-neutral-800 bg-background-secondary px-2 py-1 shadow-2xl", className)} {...props} />
 </Transition>
));

const MenuItemVariants = cva("group my-1 flex w-full cursor-pointer items-center rounded-md p-2 text-sm font-normal text-gray-400 duration-200 ui-active:text-white motion-reduce:transition-none", {
 variants: {
  variant: {
