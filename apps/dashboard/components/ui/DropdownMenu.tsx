...
const DropdownMenuContent = ({ className, sideOffset = 4, align = "start", ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Content>) => {
 return (
  <DropdownMenuPrimitive.Portal>
   <DropdownMenuPrimitive.Content sideOffset={sideOffset} align={align} className={cn("bg-background-secondary data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 mt-1 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 max-h-(--radix-dropdown-menu-content-available-height) min-w-[8rem] overflow-x-hidden overflow-y-auto rounded-2xl border border-neutral-800 px-2 py-1 shadow-md", className)} {...props} />
  </DropdownMenuPrimitive.Portal>
 );
};
...
const DropdownMenuItem = ({
 ...
 inset?: boolean;
}) => <DropdownMenuPrimitive.Item className={cn("group my-1 flex w-full focus-visible:outline-0 cursor-pointer items-center rounded-lg p-2 text-sm font-normal text-neutral-500 duration-200 hover:bg-button-primary hover:text-white focus-visible:bg-button-primary focus-visible:text-white", inset && "pl-8", className)} {...props} />;
...
const DropdownMenuSeparator = ({ className, ...props }: React.ComponentPropsWithRef<typeof DropdownMenuPrimitive.Separator>) => <DropdownMenuPrimitive.Separator className={cn("my-2 h-px bg-neutral-800", className)} {...props} />;
