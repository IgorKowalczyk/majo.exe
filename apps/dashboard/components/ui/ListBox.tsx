...
export const ListBoxButton = ({ className, ...props }: ListboxButtonProps) => <DefaultListboxButton className={cn("hover:bg-background-menu-button ui-open:border-neutral-700 ui-open:bg-background-menu-button flex h-10 cursor-pointer select-none items-center gap-3 rounded-lg border border-neutral-800 px-3 py-2 text-sm font-normal duration-200 hover:border-neutral-700 motion-reduce:transition-none sm:text-sm", className)} {...props} />;

export const ListBoxOptions = ({ className, ...props }: ListboxOptionsProps) => (
 <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
  <DefaultListboxOptions className={cn("bg-background-secondary absolute z-10 mt-1 max-h-60 min-w-(--button-width) overflow-auto origin-top-left rounded-2xl border border-neutral-800 py-1 text-base shadow-lg sm:text-sm", className)} {...props} />
 </Transition>
);
...
export const ListBoxOption = ({ children, variant, ...props }: CustomListBoxOptionProps) => (
 <DefaultListboxOption className={ListBoxOptionVariants(variant)} {...props}>
  {children}
...
export const ListBox = ({ children, ...props }: React.ComponentProps<typeof DefaultListbox>) => (
 <DefaultListbox as="div" {...props}>
  {children}
 </DefaultListbox>
);
