...
export const Textarea = React.forwardRef<HTMLTextAreaElement, React.ComponentProps<"textarea">>(({ className, ...props }, ref) => <textarea className={cn("focus:border-accent-primary! w-full rounded-lg border border-neutral-800 bg-transparent px-3 py-2 shadow-xs outline-hidden ring-0! duration-200", className)} ref={ref} {...props} />);

export const Checkbox = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(({ className, ...props }, ref) => <input type="checkbox" className={cn("checked:bg-accent-primary checked:hover:bg-accent-primary focus:border-accent-primary! focus:outline-accent-primary focus:ring-accent-primary focus:checked:bg-accent-primary cursor-pointer rounded-lg border border-neutral-800 bg-transparent p-1 shadow-xs outline-hidden ring-0! duration-200", className)} ref={ref} {...props} />);
