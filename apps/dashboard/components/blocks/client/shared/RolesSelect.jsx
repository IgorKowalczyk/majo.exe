    ...
    <div>
     <Listbox value={exemptRoles} onChange={(value) => setExemptRoles(value)} multiple={true}>
      ...
      <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
       <Listbox.Options className="bg-background-primary absolute z-[544] mt-1 max-h-60 w-fit overflow-auto rounded-md border border-neutral-800 py-1 text-base shadow-lg sm:text-sm">
        {allRoles.map((role, index) => (
         <Listbox.Option key={index} className={({ active }) => `relative mx-2 my-1 cursor-pointer select-none rounded-md py-2 pl-10 pr-4 font-normal duration-200 ${active ? "bg-accent-primary text-white" : "text-white/70"}`} value={role.id}>
          {({ selected, active }) => (
           <>
